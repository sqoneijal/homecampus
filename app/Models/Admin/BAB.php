...
class BAB extends Model {
   ...
   public function countData($post = []) {
      try {
         ...
         $table->join('tb_tingkatan tt', 'tt.id = tb.id_tingkatan');
   ...
   private function _queryData($post = []) {
      try {
         $table = $this->db->table('tb_bab tb');
         $table->select('tb.id, tb.id_tingkatan, tt.nama as nama_tingkatan, tb.nama as judul_bab, tb.keterangan');
         $table->join('tb_tingkatan tt', 'tt.id = tb.id_tingkatan');
   
         $i = 0;
         $column_search = ['tt.nama', 'tb.nama', 'tb.keterangan'];
