...
const Lists = () => {
   ...
   const hapus = (id) => {
      post("/admin/pengguna/akun/hapus", { id: id })
         .then((res) => {
            const { data } = res;
   ...
   useEffect(() => {
      datatable = $("#datatable").DataTable({
         ...
         pageLength: 100,
         ajax: {
            url: "/admin/pengguna/akun/getdata",
            type: "POST",
            data: {
               user_login: window.user.id_user,
            },
         },
         createdRow: (row, data) => {
         ...
         columns: [{ class: "text-center", orderable: false }, null, null, null, null, { class: "text-center" }, { class: "text-center", orderable: false }],
      });
      return () => {};
   }, []);
